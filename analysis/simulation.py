...
lineups = [None]*mod.num_teams
for i in range(1,mod.num_teams+1):
    lineups[i-1] = mod.probable_lineups[mod.probable_lineups.teamid == i].values[0][2:13]
...
def simulate_games(home_team, away_team, n):
    total_home = 0
    total_away = 0
    for i in range(n):
        (h, a) = simulate_game(home_team, away_team)
        total_home += h
        total_away += a
